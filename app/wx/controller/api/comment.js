...
module.exports = class extends Base {
  ...
  postAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const insertId = yield _this.model('wx_comment').add({
        type_id: typeId,
        value_id: valueId,
  ...
  countAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const allCount = yield _this2.model('wx_comment').where({ type_id: typeId, value_id: valueId }).count('id');

      const hasPicCount = yield _this2.model('wx_comment').alias('comment').join({
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      let comments = [];
      if (showType !== 1) {
        comments = yield _this3.model('wx_comment').where({
          type_id: typeId,
          value_id: valueId
        }).page(page, size).countSelect();
      } else {
        comments = yield _this3.model('wx_comment').alias('comment').field(['comment.*']).join({
          table: 'comment_picture',
          join: 'right',
          alias: 'comment_picture',
          on: ['id', 'comment_id']
      ...
      for (const commentItem of comments.data) {
        ...
        comment.user_info = yield _this3.model('wx_user').field(['username', 'avatar', 'nickname']).where({ id: commentItem.user_id }).find();
        comment.pic_list = yield _this3.model('wx_comment_picture').where({ comment_id: commentItem.id }).select();
