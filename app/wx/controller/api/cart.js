...
module.exports = class extends Base {
  ...
  getCart() {
    ...
    return _asyncToGenerator(function* () {
      const cartList = yield _this.model('wx_cart').where({ user_id: think.userId, session_id: 1 }).select();
      // 获取购物车统计信息
      let goodsCount = 0;
      ...
      for (const cartItem of cartList) {
        ...
        // 查找商品的图片
        cartItem.list_pic_url = yield _this.model('wx_goods').where({ id: cartItem.goods_id }).getField('list_pic_url', true);
      }
  ...
  addAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // 判断商品是否可以购买
      const goodsInfo = yield _this3.model('wx_goods').where({ id: goodsId }).find();
      if (think.isEmpty(goodsInfo) || goodsInfo.is_delete === 1) {
        return _this3.fail(400, '商品已下架');
      }

      // 取得规格的信息,判断规格库存
      const productInfo = yield _this3.model('wx_product').where({ goods_id: goodsId, id: productId }).find();
      if (think.isEmpty(productInfo) || productInfo.goods_number < number) {
        return _this3.fail(400, '库存不足');
      }

      // 判断购物车中是否存在此规格商品
      const cartInfo = yield _this3.model('wx_cart').where({ goods_id: goodsId, product_id: productId }).find();
      if (think.isEmpty(cartInfo)) {
        ...
        if (!think.isEmpty(productInfo.goods_specification_ids)) {
          goodsSepcifitionValue = yield _this3.model('wx_goods_specification').where({
            goods_id: goodsId,
        ...
        yield _this3.model('wx_cart').thenAdd(cartData, { product_id: productId });
      } else {
        // 如果已经存在购物车中，则数量增加
        ...
        yield _this3.model('wx_cart').where({
          goods_id: goodsId,
          product_id: productId,
          id: cartInfo.id
  ...
  updateAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // 取得规格的信息,判断规格库存
      const productInfo = yield _this4.model('wx_product').where({ goods_id: goodsId, id: productId }).find();
      if (think.isEmpty(productInfo) || productInfo.goods_number < number) {
        return _this4.fail(400, '库存不足');
      }

      // 判断是否已经存在product_id购物车商品
      const cartInfo = yield _this4.model('wx_cart').where({ id: id }).find();
      // 只是更新number
      if (cartInfo.product_id === productId) {
        yield _this4.model('wx_cart').where({ id: id }).update({
          number: number
        });
      ...
      const newCartInfo = yield _this4.model('wx_cart').where({ goods_id: goodsId, product_id: productId }).find();
      if (think.isEmpty(newCartInfo)) {
        ...
        if (!think.isEmpty(productInfo.goods_specification_ids)) {
          goodsSepcifition = yield _this4.model('wx_goods_specification').field(['wx_goods_specification.*', 'wx_specification.name']).join('wx_specification ON wx_specification.id=wx_goods_specification.specification_id').where({
        ...
        yield _this4.model('wx_cart').where({ id: id }).update(cartData);
      } else {
        ...
        yield _this4.model('wx_cart').where({ id: newCartInfo.id }).delete();
  ...
  checkedAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this5.model('wx_cart').where({ product_id: { 'in': productId } }).update({ checked: parseInt(isChecked) });
  ...
  deleteAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this6.model('wx_cart').where({ product_id: { 'in': productId } }).delete();
  ...
  checkoutAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (addressId) {
        checkedAddress = yield _this8.model('wx_address').where({ is_default: 1, user_id: think.userId }).find();
      } else {
        checkedAddress = yield _this8.model('wx_address').where({ id: addressId, user_id: think.userId }).find();
      }

      if (!think.isEmpty(checkedAddress)) {
        checkedAddress.province_name = yield _this8.model('wx_region').getRegionName(checkedAddress.province_id);
        checkedAddress.city_name = yield _this8.model('wx_region').getRegionName(checkedAddress.city_id);
        checkedAddress.district_name = yield _this8.model('wx_region').getRegionName(checkedAddress.district_id);
      ...
      // 获取可用的优惠券信息，功能还示实现
      const couponList = yield _this8.model('wx_user_coupon').select();
