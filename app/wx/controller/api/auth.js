...
module.exports = class extends Base {
  loginByWeixinAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // 解释用户数据
      const WeixinSerivce = _this.service('weixin', 'wx');
      ...
      // 根据openid查找用户是否已经注册
      let userId = yield _this.model('wx_user').where({ weixin_openid: sessionData.openid }).getField('id', true);
      if (think.isEmpty(userId)) {
        // 注册
        userId = yield _this.model('wx_user').add({
      ...
      // 查询用户信息
      const newUserInfo = yield _this.model('wx_user').field(['id', 'username', 'nickname', 'gender', 'avatar', 'birthday']).where({ id: userId }).find();

      // 更新登录信息
      userId = yield _this.model('wx_user').where({ id: userId }).update({
        last_login_time: parseInt(new Date().getTime() / 1000),
        last_login_ip: clientIp
      });

      const TokenSerivce = _this.service('token', 'wx');
