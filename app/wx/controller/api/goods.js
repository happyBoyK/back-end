...
module.exports = class extends Base {
  indexAction() {
    ...
    return _asyncToGenerator(function* () {
      const model = _this.model('wx_goods');
  ...
  skuAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const model = _this2.model('wx_goods');
  ...
  detailAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const model = _this3.model('wx_goods');

      const info = yield model.where({ 'id': goodsId }).find();
      const gallery = yield _this3.model('wx_goods_gallery').where({ goods_id: goodsId }).limit(4).select();
      const attribute = yield _this3.model('wx_goods_attribute').field('wx_goods_attribute.value, wx_attribute.name').join('wx_attribute ON wx_goods_attribute.attribute_id=wx_attribute.id').order({ 'wx_goods_attribute.id': 'asc' }).where({ 'wx_goods_attribute.goods_id': goodsId }).select();
      const issue = yield _this3.model('wx_goods_issue').select();
      const brand = yield _this3.model('wx_brand').where({ id: info.brand_id }).find();
      const commentCount = yield _this3.model('wx_comment').where({ value_id: goodsId, type_id: 0 }).count();
      const hotComment = yield _this3.model('wx_comment').where({ value_id: goodsId, type_id: 0 }).find();
      let commentInfo = {};
      if (!think.isEmpty(hotComment)) {
        const commentUser = yield _this3.model('wx_user').field(['nickname', 'username', 'avatar']).where({ id: hotComment.user_id }).find();
        commentInfo = {
          content: new Buffer(hotComment.content, 'base64').toString(),
          add_time: think.datetime(new Date(hotComment.add_time * 1000)),
          nickname: commentUser.nickname,
          avatar: commentUser.avatar,
          pic_list: yield _this3.model('wx_comment_picture').where({ comment_id: hotComment.id }).select()
        };
      }
      ...
      // 当前用户是否收藏
      const userHasCollect = yield _this3.model('wx_collect').isUserHasCollect(think.userId, 0, goodsId);

      // 记录用户的足迹 TODO
      yield yield _this3.model('wx_footprint').addFootprint(think.userId, goodsId);
  ...
  categoryAction() {
    ...
    return _asyncToGenerator(function* () {
      const model = _this4.model('wx_category');
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const goodsQuery = _this5.model('wx_goods');
      ...
      if (!think.isEmpty(keyword)) {
        ...
        // 添加到搜索历史
        yield _this5.model('wx_search_history').add({
          keyword: keyword,
      ...
      if (!think.isEmpty(categoryIds)) {
        // 查找二级分类的parent_id
        const parentIds = yield _this5.model('wx_category').where({ id: { 'in': categoryIds } }).getField('parent_id', 10000);
        // 一级分类
        const parentCategory = yield _this5.model('wx_category').field(['id', 'name']).order({ 'sort_order': 'asc' }).where({ 'id': { 'in': parentIds } }).select();
      ...
      if (!think.isEmpty(categoryId) && parseInt(categoryId) > 0) {
        whereMap.category_id = ['in', yield _this5.model('wx_category').getCategoryWhereIn(categoryId)];
      }
  ...
  filterAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const goodsQuery = _this6.model('wx_goods');

      if (!think.isEmpty(categoryId)) {
        goodsQuery.where({ category_id: { 'in': yield _this6.model('wx_category').getChildCategoryId(categoryId) } });
      }
      ...
      if (!think.isEmpty(categoryIds)) {
        // 查找二级分类的parent_id
        const parentIds = yield _this6.model('wx_category').where({ id: { 'in': categoryIds } }).getField('parent_id', 10000);
        // 一级分类
        const parentCategory = yield _this6.model('wx_category').field(['id', 'name']).order({ 'sort_order': 'asc' }).where({ 'id': { 'in': parentIds } }).select();
  ...
  relatedAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const model = _this9.model('wx_goods');
      const goodsId = _this9.get('id');
      const relatedGoodsIds = yield _this9.model('wx_related_goods').where({ goods_id: goodsId }).getField('related_goods_id');
  ...
  countAction() {
    ...
    return _asyncToGenerator(function* () {
      const goodsCount = yield _this10.model('wx_goods').where({ is_delete: 0, is_on_sale: 1 }).count('id');
