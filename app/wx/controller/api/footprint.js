...
module.exports = class extends Base {
  ...
  deleteAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // 删除当天的同一个商品的足迹
      const goods = yield _this.model('wx_footprint').where({ user_id: userId, id: footprintId }).find();
      yield _this.model('wx_footprint').where({ user_id: userId, goods_id: goods.goods_id }).delete();
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      const list = yield _this2.model('wx_footprint').field(['f.*', 'g.name', 'g.list_pic_url', 'g.goods_brief', 'g.retail_price']).alias('f').join({
        table: 'goods',
        join: 'left',
        as: 'g',
        on: ['f.goods_id', 'g.id']
